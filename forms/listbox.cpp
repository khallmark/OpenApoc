...
bool ListBox::selectItemByIndex(size_t index)
{
	...
	// ListBoxChangeSelected (see eventOccured above), which is what screens actually listen to --
	// ResearchScreen swaps the active lab on it, for one. setSelected() raises nothing, and
	// Control::click() on the item raises MouseClick, which ListBox does not treat as selection
	// at all. A harness "set" built on either reports success and changes nothing.
