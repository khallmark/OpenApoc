...
{
	// Controls are constructed by tools and tests that never bring up a display.
...
bool Control::isPointInsideControlBounds(int x, int y) const
{
	...
	// x/y arrive in display pixels; the control's bounds are in UI space.
...
void Control::render()
{
	...
	// Only a top-level control converts: a child's Location is relative to its parent, and
	// the parent's own draw already placed the whole subtree.
	...
	const Colour disabledTint(255, 255, 255, 128);
	if (destSize == Size)
	{
		if (Enabled)
		{
		...
		{
			fw().renderer->drawTinted(controlArea, destPos, disabledTint);
		}
	}
	else
	{
		if (Enabled)
		{
			fw().renderer->drawScaled(controlArea, destPos, destSize, Renderer::Scaler::Nearest);
		}
		else
		{
			fw().renderer->drawScaledTinted(controlArea, destPos, destSize, disabledTint);
		}
	}
}
