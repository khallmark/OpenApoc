// Regression lock for two large-unit ("bodyType->large", e.g. Megaspawn/Multiworm) behaviors
// extracted from game/state/battle/battleunit.cpp / .h as a single translation-unit PR:
//
//   OPE-9  -- multi-tile line of sight: visibility/targeting against a large unit must succeed
//            if ANY of its occupied tiles has a clear line, not just one degenerate probe point.
//            Fixed in this PR: BattleUnit::calculateVisionToUnit(), BattleUnit::hasLineToUnit(),
//            and BattleUnit::getEyeLocation() (the "ground-snag" half of the same bug: a large
//            unit's own eyes were half a tile off from its own muzzle).
//
//   OPE-12 -- large-unit footprint occupancy and narrow-gap/ground-snag pathing. This PR does
//            NOT fix the occupancy bug: the actual production fix is a one-line change in
//            game/state/tilemap/tileobject_battleunit.cpp (TileObjectBattleUnit::setPosition()
//            fails to register a moving large unit's own goal-block corner tile), which is a
//            different translation unit and out of scope for this file-unit PR. The narrow-gap
//            pathing check below needs no fix at all -- BattleUnitTileHelper::canEnterTile()
//            (game/state/battle/battleunitmission.cpp) already rejects a destination block that
//            doesn't fully fit, on both upstream master and develop -- so it is included here as
//            a plain regression lock, not a bug fix. See the PR body for the full writeup.
//
// Both behaviors are bundled in one file because both tickets' production code lives in the
// same translation unit (battleunit.cpp/.h); splitting them would create textual conflicts
// between sibling PRs in the same campaign. Fixtures below are grouped and labelled by behavior
// so a reviewer can tell which is which at a glance.
//
// This test intentionally does not include "tests/test_helpers.h": that shared seam is only
// introduced by a sibling, not-yet-merged campaign PR (establishing per-slice test auto-
// discovery), and this file's own unit boundary is battleunit.cpp/.h plus this one new test file.
// The small TEST_CHECK/TEST_REQUIRE/runTestSuite/gamestate-loading helpers below are therefore
// self-contained duplicates of that seam's shape, scoped to this file only.
//
// Each test uses a real, procedurally generated Battle (Battle::beginBattle()/enterBattle() from
// an actual UFO's battle_map) rather than a hand-built TileMap: BattleUnit::setFacing() ->
// refreshUnitVision() -> calculateVisionToTerrain() unconditionally indexes
// battle.visibleBlocks/visibleTiles/losBlocks, which only BattleMap's real map generation
// populates, and BattleUnit::setBodyState() needs a unit's image/animation packs loaded, which
// Battle::initBattle() only does for participants already present when it runs. What stays
// fully controlled is the geometry actually under test: canEnterTile()/Tile::getPassable() only
// ever consult a Tile's raw scalar fields (never the scenery graph), so poking those directly
// scripts a deterministic pathfinding scenario regardless of what the randomly generated map
// underneath happens to contain. The LOS test similarly fabricates its own solid
// BattleMapPartType/VoxelMap rather than hunting for a suitably placed real wall.
...
// =================================================================================================
// OPE-12 fixtures -- large-unit footprint occupancy and constrained (narrow-gap) pathing.
// Geometry-only: no line of sight involved.
// =================================================================================================
...
// block it occupies at rest. This locks exactly that stationary case, which is already correct
// on upstream master and is NOT changed by this PR.
//
// NOT locked here: while moving, setPosition is also supposed to register the union of the
// current and goal blocks, but it drops the goal block's own floor-level corner tile from that
// union (see game/state/tilemap/tileobject_battleunit.cpp, TileObjectBattleUnit::setPosition).
// The fix is a one-line change in that file, which is a different translation unit from
// battleunit.cpp/.h and out of scope for this file-unit PR -- see the PR body's OPE-12 section.
...
//
// This is a pure regression lock, not a bug fix: canEnterTile already rejects the 1-tile gap on
// unmodified upstream master, and this PR makes no production change to it or to any file other
// than battleunit.cpp/.h.
...
// =================================================================================================
// OPE-9 fixtures -- large-unit multi-tile line of sight. Sight-line-only: no pathing/occupancy
// assertions involved (occupiedTiles itself is not inspected by either test below).
// =================================================================================================
...
// battleunit.h's former "FIXME: This likely won't work properly for large units" and the "caught
// up in ground" workaround comments (battleunit.cpp) were compensating for at the LOS/line-of-
// fire call sites: the aim points a large unit's own vision and gunfire converge on did not agree
// with each other, and did not sit at the block's true centre. Fixed in this PR.
...
// target is visible. The as-found implementation instead probed a single point (the block's
// exact geometric centre, nudged 0.75 tiles toward the viewer) rather than the target's actual
// occupied tiles. This locks the "any occupied tile" contract against the real map/collision
// machinery: a solid wall placed in front of the near half of the target's footprint, but not the
// far half, must not make the target invisible. Fixed in this PR.
//
