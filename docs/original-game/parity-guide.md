...
| **U1(b)** `+0x168` vs constitution gate | **MECHANISM BOUND, SEMANTICS NOT** | The `CMP AX,[EBP+0x12e]; JLE` pair is verified and three spawn-time writers traced — but **no writer raises `+0x168` after spawn**, which rules out the "regenerating health" reading a first pass suggests. What the field means is still unknown. **Not guessed.** |
