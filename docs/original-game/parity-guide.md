...
| **G1 · Disruptor Shield (0x08)** | **FIXED — and the premise was wrong** | *Not* a missing feature. `useItem` returning `false` is **correct** — the shield is passive, like `CloakingField`. A shield-absorption path has existed since 2016 (`Agent::getFirstShield`, and the "Hit shield if present" block at `battleunit.cpp:1776`); it was simply **wrong**: it always returned true after any hit (infinite absorption, no overflow), used the item's own `damage_modifier` instead of the general pipeline, and destroyed the item on depletion despite the shield being rechargeable. Chain traced to the general damage-application function by caller trace. All four numbers now bound: regen **+1 per 36 vanilla ticks** (one game-second under the repository's selected observational 36-TPS canon; the binary binds the interval, not absolute cadence); full recharge **at battle load only, not periodic**; damage-type modifier is **not shield-specific** (existing `damage_type_data` + a table adjacent to `damage_modifier_data`, applied upstream); and absorption is **all-or-nothing** — see the trap below. **Second pass:** only one of the two bound writers of the item's charge field had been implemented. `FUN_0006508C`'s decrement on absorption was in, but `FUN_0006511C` — which regenerates the item's own charge by 1 per dispatcher call, gated `< 100` — was not, so absorption lowered the item charge while regen raised only the unit buffer and the two drifted apart; unequip/re-equip snapped a fully regenerated shield back to its drained value. Also excluded the shield from `AEquipment::updateInner()`'s generic recharge, which was a *second* regen of the same field at one per four selected-canon game-seconds, from hardcoded 2016 extractor literals rather than the binary. |
| **G1 · MultiTracker (0x04)** | **BOUND — and it is NOT a dead gadget** | `FUN_000A3170` is a literal `type == 0x04` test gating a live display: an **instantaneous, non-decaying** snapshot of nearby matching-tag units' positions (height-encoded) drawn into a fixed 81×81 widget, redrawn from scratch each refresh with no accumulation state. The earlier "registered candidate list" reading was a **decompiler artifact** — the two call sites pass literal `0`/`1`, a pre-pass/post-pass mode flag, not a data pointer. Motion Scanner's structurally-parallel path is a *different* mechanic: it blits a separately-maintained 15-level **decaying** recency heatmap, which is what OpenApoc's `BattleScanner::movementTicks` already models. **OpenApoc currently groups MultiTracker with the confirmed-dead gadgets; that grouping is wrong.** A port must not reuse `BattleScanner`'s fade model — the original has no decay at all. Left unclaimed: the wearer/candidate tag-equality fields (`unit+0x20`/`+0xA8`). |
| **G1 · MultiTracker (0x04)** *(superseded)* | **BOUND** *(upgraded from inconclusive)* | Traced one hop past the local cluster to a builder with six live call sites across five functions, invoked in the same init block as the confirmed-live Motion Scanner chain and behind the same feature flag. A real shared subsystem, not dead code. |
...
| **B1** cover metric | **IMPLEMENTED** | Bound on pass 2 and built. The recovered mechanism is **not** what this guide originally guessed: `FUN_0008c1fc` does not sweep neighbouring tiles for solidity, it scores a short fixed **menu** of candidate destinations by threat exposure (`FUN_0007e600`, 21×21×13 box, accumulator from 0 downward) and max-selects the least-exposed. `UnitAIHelper::exposureScore()` implements that as a pure static; `getTakeCoverMovement()` no longer returns `nullptr`. Deliberately a **count**, not a weighted sum — the original's range/facing penalty factor was never traced, so the magnitude is unrecovered while the ordering is. **No-cheat is structural:** `exposureScore` has no `GameState` access at all and its only caller fills the threat list from `Battle::visibleEnemies[u.owner]`, which is LOS-derived. Disclosed gap: the original's menu also reads up to four pre-authored per-map AI waypoints from a table this engine does not extract. |
| **B2** evasive seek-cover | **IMPLEMENTED — prior-art, labelled** | Unblocked by B1. `cautious` was already threaded through; the real gap was arrival stance — a cover move set `Running`/no-kneel regardless, so an Evasive unit sprinted to cover and stood up in it. Now prones there when `canProne`, else kneels on arrival. **This is prior-art, not recovered:** ai.txt distinguishes the modes only by the no-cover fallback, there is no printable `evasive` in TACP, and nothing in the recovered chain specifies an arrival stance. Marked as such at the point of use. |
| **B1** cover metric *(superseded)* | **RE-OPENED** | First verdict rested on an invalid control; see below. Agent resumed with structural entry. |
