...
6. Record gaps in [openapoc-gap-matrix.md](openapoc-gap-matrix.md); [parity-guide.md](parity-guide.md) turns each open row into a work item with an RE procedure, implementation steps and a lock test. The finished extract↔runtime compare is [compare-report.html](compare-report.html) (pass 5 is the bound xref census: 24/14 live xrefs, gap needles empty). Extractor index: [extractor-tables.md](extractor-tables.md).
