...
9. **`UFO_mission_data` tail leftovers** — `follow_slot`, spawn XY, `type_percent` constitution multiply, and the scatter>50 clamp are consumed. `building_function` is a byte index copied to vehicle `+0x171` (`FUN_0006da88` @ file `0xD030B`) and consumed by `FUN_0006d384` @ file `0xCFD53` against org tables VA `0x1439E0` (stride `0x2D4`; static EXE bytes are zero — pure runtime RAM). Chain: `FUN_0006cfb4` @ file `0xBF658` picks the incursion row; `FUN_000706f8` @ file `0xC06F8` rebuilds `0x1439E0` coords (`+4`/`+0x18`/`+0x2c` xyz, `FUN_0005ce00` empty-tile check); `FUN_0006d384` `DAT_000d505e` pass walks live vehicles with `+0x171` and calls `FUN_0005e7f4` over the target box; `FUN_0006eea4` @ file `0xCEEA4` copies table[`+0x171`] when `!=0xFF` else random search (`FUN_0005ce00` + `FUN_0005e7f4`); `FUN_000588f8` @ file `0x58925` gates `+0x168` vs `+0x12e` (constitution threshold). `FUN_0006c058` @ file `0xBC058` seeds table dimensions (20-slot loop). `FUN_0005e7f4`, `FUN_0006c058` dimension writers, and `FUN_000588f8` consumer stay unbound. Do not invent an `acquireTargetBuilding` name/`function` pick; `+0x171` is later a retry counter (`FUN_0003a910`).
