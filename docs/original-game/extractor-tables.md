...
| `agent_equipment` / `agent_armor` / `agent_weapon` / `agent_general` / `agent_payload` / `fire_hazard_power_table` | `aequipment.h` | Consumed. `unknown01` → `hazardResist` (`FUN_0007c110` @ file `0xD6BB4`). Power table `0x2E2AF4` (`FUN_0007ae18` @ file `0xD58BC`) drives item contact for recovered overlays. `BattleHazard::fireOverlay` preserves type 2 + the six-bit index (`FUN_0007ad94`), including preplaced marker 25 → `0x99`. `Battle::updateFireScheduler` implements the real-time `FUN_0007b7f8` global scheduler: four OpenApoc ticks per vanilla iteration, `(mapY×mapZ)/0x48` X rows, persistent Y/Z cursors, then contact counter check/reset/increment at `0x24`. Row progression uses `FUN_0007b3dc` extinction and terrain `fire_burn_time`; item contact uses table power. Turn-based scheduling, unit fire intensity, generic placement, and spread RNG (`FUN_0001eee8` / `FUN_0007b0d0`) remain unbound; legacy hazards keep the old path and `HAZARD_SPREAD_CHANCE` is still made up. |
