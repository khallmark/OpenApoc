...
| `agent_equipment` / `agent_armor` / `agent_weapon` / `agent_general` / `agent_payload` / `fire_hazard_power_table` | `aequipment.h` | Consumed. `unknown01` → `hazardResist` (`FUN_0007c110` @ file `0xD6BB4`). Power table `0x2E2AF4` (`FUN_0007ae18` @ file `0xD58BC`) is used for item contact when a recovered overlay is present. `BattleHazard::fireOverlay` preserves type 2 + the six-bit index (`FUN_0007ad94`), including preplaced marker 25 → `0x99`; pure helpers lock `FUN_0007b3dc` progression/extinction and the terrain `fire_burn_time` comparison. Runtime global row scheduling, unit fire intensity, generic placement, spread RNG (`FUN_0001eee8` / `FUN_0007b0d0`), and exact contact cadence remain unbound; legacy hazards keep the old path and `HAZARD_SPREAD_CHANCE` is still made up. |
