...
| `building_names` / `building_functions` / `alien_building_names` | `building.h` | Consumed. `agentSpawnType` is extracted and serialized; no UFOPaedia or simulation reader. |
...
| `agent_equipment` / `agent_armor` / `agent_weapon` / `agent_general` / `agent_payload` / `fire_hazard_power_table` | `aequipment.h` | Consumed. `unknown01` → `hazardResist` (`FUN_0007c110` @ file `0xD6BB4`). Power table `0x2E2AF4` (`FUN_0007ae18` @ file `0xD58BC`). Overlay index is `overlay&0x3F` (`FUN_0007ad94`); `FUN_0007b3dc` increments the overlay byte. Placement RNG `FUN_0001eee8` unbound. OpenApoc `age`/`power` are not that index. |
...
| ----------- | ------------ | ------- | ------------------- |
| `UFO_mission_data` | `0x13DDFC` (VA `0x1119FC`) | extracted | `extract_ufo_incursions.cpp` (45×42; follow-slot + `FUN_0003b724` spawn XY @ `0x2B723`; `type_percent` × constitution; scatter>50→10 clamp; `building_function` / `+0x168` gate extracted-only). Reference: `docs/original-game/exe-tables/ufo_incursions.xml` (must not live under `common_patch/gamestate/`). |
