...
driver can begin from a cold process with `--cold-main-menu` instead of creating unrelated
campaign state. Ordinary use still starts a game and enters through InGameOptions.
...
**The complete cold battle is currently a known-red acceptance:**

```sh
python3 tools/oa_skirmish.py --cold-main-menu --rounds 1
```

The framework drains a snapshot of its pending `StageCmd` queue. When a stage's `resume()` queues
another command during that drain, the new command is later cleared rather than processed. After
AEquipScreen pops, `SelectForces::resume()` queues its own POP and remains stuck on SelectForces.
If that screen is manually popped, `Skirmish::resume()->loadBattle()` queues the battle transition
during the same kind of drain and remains stuck on Skirmish. The live-FIFO StageCmd PR must turn
this exact path into a real BattleView smoke; until then, reaching either stuck stage is setup
failure, not evidence of a tactical loss. A separately observed battlemap-generation exception is
likewise a setup/engine failure, not a gameplay outcome.
